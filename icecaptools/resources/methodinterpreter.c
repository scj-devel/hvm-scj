...
static const unsigned char* getCodePointer(const MethodInfo* currentMethod);
...
static int32 methodInterpreter(unsigned short currentMethodNumber, int32* fp) {
	...
	start: {
		...
		method_code = getCodePointer(currentMethod);
	...
	loop: while (1) {
...
#if defined(ENABLE_DEBUG)
		checkBreakpoint(fp, currentMethodNumber, method_code - getCodePointer(currentMethod));
#endif
		switch (code) {
		...
		case IRETURN_OPCODE: {
			...
			if (currentMethodNumber != 0) {
				...
				currentMethodNumber--;
				currentMethod = &methods[currentMethodNumber];
				method_code = getCodePointer(currentMethod) + pc;
...

		{
			...
			unsigned short pc = method_code - getCodePointer(currentMethod);
...
extern const unsigned char* const codecache;

const unsigned char* getCodePointer(const MethodInfo* currentMethod) {
	uint32 offset = pgm_read_pointer(&currentMethod->codeOffset, uint32*);
	if (offset > 0) {
		const unsigned char* code;
		code = &codecache[offset];
		return code;
	} else {
		return 0;
	}
