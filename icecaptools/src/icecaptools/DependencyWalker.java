...
public class DependencyWalker {
    ...
    private void analyseDynamicMethodCallBNode(DynamicMethodCallBNode bnode, NewList newList, NewList mr, Stack<WorkItem> workItemStack) throws Throwable {
        if (bnode instanceof DynamicMethodCallVirtualMethodBNode) {
            ...
            observer.methodCodeUsed(virtualMethodcallType.getClassNameHandle(), virtualMethodcallType.getMethodNameHandle(), virtualMethodcallType.getMethodSigHandle(), true);
            observer.classUsed(virtualMethodcallType.getClassNameHandle());

            MethodEntryPoints entryPoints = converter.convertByteCode(bnode, clazz, virtualMethodcallType.getMethodNameHandle(), virtualMethodcallType.getMethodSigHandle(), false);
    ...
    private void handleThreadStart(MethodCallBNode bnode, NewList newList, NewList mr) throws Throwable {
        ...
        if (clazz != null) {
            MethodEntryPoints entryPoints = converter.convertByteCode(bnode, clazz, "dispatchRunnable", "(Ljava/lang/Runnable;)V", true, true);
        ...
        while (count < elements.length) {
            ...
            if (!JavaArrayClass.isArrayClass(className)) {
                ...
                if (checker.isSubclassOf(subClazz, clazz)) {
                    subClazz = findMethodInClassHierarchy(subClazz, bnode.getMethodName(), bnode.getMethodSig()).getClazz();
