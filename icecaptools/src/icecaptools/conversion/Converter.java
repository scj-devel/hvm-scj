...
public class Converter implements ClassManager {
	...
	public Converter(PrintStream out, RestartableMethodObserver methodObserver, ICompilationRegistry cregistry,
			boolean supportLoading) throws ClassNotFoundException {
		this.out = out;
	...
	public NewList startConversion(ConversionConfiguration config, Iterator<String> classInitializers) {
		...
		try {
			...
			org.apache.bcel.util.Repository repository = new IcecapRepository(
			...
			while (classInitializers.hasNext()) {
				clazz = dependencyWalker.lookupClass(classInitializers.next());
			...
			clazz = dependencyWalker.lookupClass(config.getInputPackage() + "." + config.getInputClass());
			...
			entryPoint = convertByteCode(null, clazz, config.getEntryPointMethodName(),
					config.getEntryPointMethodSignature(), config.isEntryPointModyfierIsStatic());
	...
	public MethodEntryPoints convertByteCode(BNode cause, JavaClass targetClass, String targetMethodName,
			String targetMethodSignature, boolean isStatic) throws Exception {
		return convertByteCode(cause, targetClass, targetMethodName, targetMethodSignature, isStatic, false);
	}

	public MethodEntryPoints convertByteCode(BNode cause, JavaClass targetClass, String targetMethodName,
			String targetMethodSignature, boolean isStatic, boolean forceInclusion) throws Exception {
		...
		if (result == null) {
			...
			if (result != null) {
				dependencyExtent.insertMethod(result, targetClass.getClassName(), targetMethodName,
	...
	private MethodEntryPoints convert(JavaClass clazz, String targetMethodName, String targetMethodSignature,
			...
			if (code != null) {
				...
				while (stream.hasMore()) {
					...
					if (current instanceof Raw_goto) {
						...
						currentBNode = new GotoBNode(rawGoto.getAddress(), (int) rawGoto.branchoffset,
						...
						currentBNode = new BranchBNode(shortBranchOperation.getAddress(),
								(int) shortBranchOperation.branchoffset, clazz.getClassName(), targetMethodName,
								targetMethodSignature);
						...
						currentBNode = new NewBNode(rawNew.getAddress(), newType, clazz.getClassName(),
								targetMethodName, targetMethodSignature);
					} else if (current instanceof Raw_checkcast) {
						JavaClass type = ClassfileUtils.getType(clazz, ((Raw_checkcast) current).cpIndex);
						if (type != null) {
							currentBNode = new CheckcastBNode(current.getAddress(), type, clazz.getClassName(),
									targetMethodName, targetMethodSignature);
						} else {
							currentBNode = new BasicBNode(current.getAddress(), clazz.getClassName(), targetMethodName,
									targetMethodSignature);
						}
						...
						ConstantClass classRef = (ConstantClass) clazz.getConstantPool()
								.getConstant(methodRef.getClassIndex());
						ConstantNameAndType nameAndType = (ConstantNameAndType) clazz.getConstantPool()
								.getConstant(methodRef.getNameAndTypeIndex());
						ConstantUtf8 className = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(classRef.getNameIndex());
						ConstantUtf8 methodName = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(nameAndType.getNameIndex());
						ConstantUtf8 methodSig = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(nameAndType.getSignatureIndex());
						currentBNode = new VirtualMethodCallBNode(rawInvokeVirtual.getAddress(), className.getBytes(),
								methodName.getBytes(), methodSig.getBytes(), clazz.getClassName(), targetMethodName,
								targetMethodSignature);
						...
						ConstantInvokeDynamic constantInvokeDynamic = (ConstantInvokeDynamic) clazz.getConstantPool()
								.getConstant(index);
							...
							if (attribute instanceof BootstrapMethods) {
								...
								if (bmethodsAsArray != null) {
									if (bootstrapIndex < bmethodsAsArray.length) {
										...
										ConstantMethodHandle methodRef = (ConstantMethodHandle) clazz.getConstantPool()
										...
										case RawByteCodes.REF_newInvokeSpecial: {
											ConstantMethodref methodInfo = (ConstantMethodref) clazz.getConstantPool()
													.getConstant(methodRef.getReferenceIndex());
											ConstantClass classRef = (ConstantClass) clazz.getConstantPool()
													.getConstant(methodInfo.getClassIndex());
											ConstantNameAndType nameAndTypeHandle = (ConstantNameAndType) clazz
													.getConstantPool().getConstant(methodInfo.getNameAndTypeIndex());
											ConstantUtf8 classNameHandle = (ConstantUtf8) clazz.getConstantPool()
													.getConstant(classRef.getNameIndex());
											ConstantUtf8 methodNameHandle = (ConstantUtf8) clazz.getConstantPool()
													.getConstant(nameAndTypeHandle.getNameIndex());
											ConstantUtf8 methodSigHandle = (ConstantUtf8) clazz.getConstantPool()
													.getConstant(nameAndTypeHandle.getSignatureIndex());

											// System.out.println(classNameHandle
											// + ":" + methodNameHandle + ":" +
											// methodSigHandle);
											currentBNode = new DynamicMethodCallVirtualMethodBNode(
													rawInvokeDynamic.getAddress(), bootstrapIndex,
													methodName.getBytes(), methodSig.getBytes(),
													classNameHandle.getBytes(), methodNameHandle.getBytes(),
													methodSigHandle.getBytes(), clazz.getClassName(), targetMethodName,
													targetMethodSignature);
										...
										for (int i : args) {
											...
											if (c instanceof ConstantMethodType) {
												...
												case RawByteCodes.REF_invokeSpecial:
												case RawByteCodes.REF_newInvokeSpecial: {
													ConstantMethodref methodInfo = (ConstantMethodref) clazz
															.getConstantPool()
															.getConstant(methodRef.getReferenceIndex());
													ConstantClass classRef = (ConstantClass) clazz.getConstantPool()
															.getConstant(methodInfo.getClassIndex());
													ConstantNameAndType nameAndTypeHandle = (ConstantNameAndType) clazz
															.getConstantPool()
															.getConstant(methodInfo.getNameAndTypeIndex());
													ConstantUtf8 classNameHandle = (ConstantUtf8) clazz
															.getConstantPool().getConstant(classRef.getNameIndex());
													ConstantUtf8 methodNameHandle = (ConstantUtf8) clazz
															.getConstantPool()
															.getConstant(nameAndTypeHandle.getNameIndex());
													ConstantUtf8 methodSigHandle = (ConstantUtf8) clazz
															.getConstantPool()
															.getConstant(nameAndTypeHandle.getSignatureIndex());
													...
													if (currentBNode instanceof DynamicMethodCallVirtualMethodBNode) {
														((DynamicMethodCallVirtualMethodBNode) currentBNode)
																.setLambdaMethod(classNameHandle.getBytes(),
																		methodNameHandle.getBytes(),
																		methodSigHandle.getBytes());
													}
													break;
												}
												case RawByteCodes.REF_invokeInterface:
						...
						ConstantUtf8 methodSig = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(nameAndType.getSignatureIndex());
						currentBNode = new SpecialMethodCallBNode(rawInvokeSpecial.getAddress(), className.getBytes(),
								methodName.getBytes(), methodSig.getBytes(), clazz.getClassName(), targetMethodName,
								targetMethodSignature);
						...
						ConstantUtf8 methodSig = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(nameAndType.getSignatureIndex());
						currentBNode = new StaticMethodCallBNode(rawInvokeStatic.getAddress(), className.getBytes(),
								methodName.getBytes(), methodSig.getBytes(), clazz.getClassName(), targetMethodName,
								targetMethodSignature);
						...
						ConstantInterfaceMethodref methodRef = (ConstantInterfaceMethodref) clazz.getConstantPool()
								.getConstant(index);
						ConstantClass classRef = (ConstantClass) clazz.getConstantPool()
								.getConstant(methodRef.getClassIndex());
						ConstantNameAndType nameAndType = (ConstantNameAndType) clazz.getConstantPool()
						...
						ConstantUtf8 methodSig = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(nameAndType.getSignatureIndex());
						currentBNode = new InterfaceMethodCallBNode(rawInvokeInterface.getAddress(),
								className.getBytes(), methodName.getBytes(), methodSig.getBytes(), clazz.getClassName(),
								targetMethodName, targetMethodSignature);
					} else if ((current instanceof RawReturnOperation) || (current instanceof Raw_ret)
							|| (current instanceof Raw_athrow)) {
						currentBNode = new ReturnBNode(current.getAddress(), clazz.getClassName(), targetMethodName,
								targetMethodSignature);
						...
						ConstantClass classIndex = (ConstantClass) clazz.getConstantPool()
								.getConstant(fieldRef.getClassIndex());
						ConstantUtf8 className = (ConstantUtf8) clazz.getConstantPool()
								.getConstant(classIndex.getNameIndex());
						ConstantNameAndType nameAndType = (ConstantNameAndType) clazz.getConstantPool()
						...
						currentBNode = new StaticFieldAccessBNode(putgetstatic.getAddress(),
								className.getBytes().replace('/', '.'), fieldName, signature,
								current instanceof Raw_getstatic, clazz.getClassName(), targetMethodName,
								targetMethodSignature);
					} else if (current instanceof Raw_tableswitch) {
						Raw_tableswitch tableSwitch = (Raw_tableswitch) current;
						currentBNode = new SwitchBNode(tableSwitch, tableSwitch.getAddress(), tableSwitch.defaultVal,
								clazz.getClassName(), targetMethodName, targetMethodSignature);
						...
						currentBNode = new SwitchBNode(lookupSwitch, lookupSwitch.getAddress(), lookupSwitch.defaultVal,
								clazz.getClassName(), targetMethodName, targetMethodSignature);
						...
						String fieldName = nameAndType.getName(clazz.getConstantPool());
						String signature = nameAndType.getSignature(clazz.getConstantPool());
						currentBNode = new ObjectFieldAccessBNode(current.getAddress(), className.getBytes(), fieldName,
								signature, current instanceof Raw_getfield, clazz.getClassName(), targetMethodName,
								targetMethodSignature);
					} else if ((current instanceof Raw_ldc) || (current instanceof Raw_ldc_w)
							|| (current instanceof RawByteCodes.Raw_ldc2_w)) {
						int cpIndex;
						...
						currentBNode = new LDCBNode(constant, current.getAddress(), clazz.getClassName(),
								targetMethodName, targetMethodSignature);
						...
						currentBNode = new NewArrayBNode(newType, newArray.getAddress(), clazz.getClassName(),
								targetMethodName, targetMethodSignature);
					} else if (current instanceof Raw_newArray) {
						Raw_newArray newArray = (Raw_newArray) current;
						currentBNode = new NewArrayBNode(newArray.type, newArray.getAddress(), clazz.getClassName(),
								targetMethodName, targetMethodSignature);
						...
						if (newType.isBasicType()) {
							...
							currentBNode = new NewArrayMultiBNode(type, newArray.getAddress(), clazz.getClassName(),
									targetMethodName, targetMethodSignature, newType.getDimension());
						} else {
							currentBNode = new NewArrayMultiBNode(newType, newArray.getAddress(), clazz.getClassName(),
									targetMethodName, targetMethodSignature, newType.getDimension());
						}
					} else {
						currentBNode = new BasicBNode(current.getAddress(), clazz.getClassName(), targetMethodName,
	...
	public NewList startConversion(ConversionConfiguration config) {
		return startConversion(config, new EmptyStringIterator());
	}
}
