...
public class FieldOffsetCalculator {
	...
	private OffsetPair calculateOffset(JavaClass clazz, HashMap<JavaClass, ArrayList<FieldInfo>> objectFields,
			HashMap<JavaClass, ArrayList<FieldInfo>> classFields, boolean includeClassFields, AnalysisObserver observer,
			LinkedList<String> heirarchy, ArrayList<String> additionalDependencies) throws Exception {
		...
		if (superClass != null) {
			currentObjectOffset = calculateOffset(clazz.getSuperClass(), objectFields, classFields, true, observer,
					heirarchy, additionalDependencies);
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (!fields[i].isStatic()) {
				...
				// }
				objectFieldSet.add(new FieldInfo(fieldName, fieldOffset, fieldSize, fields[i].isVolatile(),
						fields[i].getType() instanceof ReferenceType, fieldType == Type.FLOAT, false, isHWObject));
			} else {
				if (includeClassFields) {
					if (observer.isClassFieldUsed(clazz.getClassName(), fieldName)) {
						fieldOffset = currentClassOffset;
						FieldInfo fInfo = new FieldInfo(fieldName, fieldOffset, fieldSize, false,
								fields[i].getType() instanceof ReferenceType, fieldType == Type.FLOAT, true, false);
	...
	private boolean isNativeField(AnalysisObserver observer, String containingClass, int fieldOffset, Field field,
			FieldInfo fInfo) {
