...
{
	e_serial_return_code_t result = serial_send_byte(_bt_serial_instance, byte);
	...
	return BT_BUFFER_FULL;
}

// ----------------------------------------------------------------------------------------------------------------------
bt_return_code_t bt_send_bytes (uint8_t *bytes, uint8_t len)
{
	e_serial_return_code_t result = serial_send_bytes (_bt_serial_instance, bytes, len);
...
bt_return_code_t bt_get_byte(uint8_t * byte) {
	if ( serial_get_byte (_bt_serial_instance,  byte ) == SERIAL_OK )
	{
		return BT_SUCCESS;
	}

	return BT_BUFFER_EMPTY;
}
...
static void _bt_status_call_back (uint8_t result)
{
	...
	if (result == DIALOG_OK_STOP) {
		serial_flush_buffers(_bt_serial_instance);
	}
	
	if (_app_bt_status_call_back) {
		if (result == DIALOG_OK_STOP)
		{
			_app_bt_status_call_back (BT_CONNECTED, 0);
		}
		else
		{
			_app_bt_status_call_back (BT_ERROR_DURING_CONNECTION, 0);
		}
	}
}
...
static void _bt_call_back (serial_p bt_serial_instance, uint8_t serial_last_received_byte)
{
	...
	{
		if (_app_bt_status_call_back != NULL)
		{
