...
public class TestSCJManagedMemory
{
  ...
  static class MissionStub extends Mission 
  {
    ...
    { 
      ...
      if (!( TestPortal.ManagedMemory_getOuterMemory ( (ManagedMemory)MemoryArea.getMemoryArea(obj) ) 
             instanceof ManagedMemory.ImmortalMemory )) 
        errors++;
      
      // missSeq is in immortal
      ManagedMemory.executeInAreaOf(missSeq, 
        new Runnable() 
        {
           public void run()
           {
...
//          });
      ...
      PeriodicEventHandler pevh = new PeriodicEvhStub(
...
//        devices.Console.println("Mem " + i + " : " + memRecord[i]);
  ...
  { 
    ...
    public void handleAsyncEvent() 
    {
      ...
      ManagedMemory.executeInAreaOf(this.missSeq, 
          new Runnable() 
          {
             public void run()
             {
      ...
      ManagedMemory.executeInAreaOf(this, 
          new Runnable() 
          {
             public void run()
             {
...
//    vm.Memory.startMemoryAreaTracking();
    ...
    storageParameters_Sequencer = new StorageParameters(Const.OUTERMOST_SEQ_BACKING_STORE,
        new long[] { 2*Const.HANDLER_STACK_SIZE }, Const.PRIVATE_MEM + 3, Const.IMMORTAL_MEM, Const.MISSION_MEM);
    ...
    new Launcher(new SafeletStub(), 2);
