...
void dumpNativeFunc(int16(*nativeFunc)(int32 *sp), const char* functionName) {
    ...
    } else if (nativeFunc == n_java_lang_Object_notify) {
        dumpByte(N_JAVA_LANG_OBJECT_NOTIFY_NUM);
    } else if (nativeFunc == n_java_lang_Object_wait_) {
        dumpByte(N_JAVA_LANG_OBJECT_WAIT__NUM);
    ...
    } else if (nativeFunc == n_vm_Monitor_attachMonitor) {
        dumpByte(N_VM_MONITOR_ATTACHMONITOR_NUM);
    } else if (nativeFunc == n_vm_Monitor_getAttachedMonitor) {
        dumpByte(N_VM_MONITOR_GETATTACHEDMONITOR_NUM);
